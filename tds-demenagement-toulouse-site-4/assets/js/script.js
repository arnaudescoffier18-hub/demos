...
(()=>{
  ...
  // Phone-only smart header: scroll down hides it, scroll up reveals it.
  const siteHeader=q('.site-header'), phoneHeaderMQ=matchMedia('(max-width: 760px)');
  if(siteHeader){
    let lastY=Math.max(0,window.scrollY), directionStartY=lastY, lastDirection=0, headerTicking=false;
    const updateMobileHeader=()=>{
      const y=Math.max(0,window.scrollY);
      const direction=y>lastY?1:y<lastY?-1:0;
      const menuOpen=!!(mobile&&mobile.classList.contains('open'));
      if(!phoneHeaderMQ.matches||y<=12||menuOpen){
        siteHeader.classList.remove('mobile-header-hidden');
        directionStartY=y;
        lastDirection=direction;
      }else if(direction){
        if(direction!==lastDirection){directionStartY=y;lastDirection=direction;}
        const distance=Math.abs(y-directionStartY);
        if(direction===1&&distance>14)siteHeader.classList.add('mobile-header-hidden');
        if(direction===-1&&distance>9)siteHeader.classList.remove('mobile-header-hidden');
      }
      lastY=y;headerTicking=false;
    };
    const requestMobileHeader=()=>{if(!headerTicking){headerTicking=true;requestAnimationFrame(updateMobileHeader)}};
    addEventListener('scroll',requestMobileHeader,{passive:true});
    addEventListener('resize',requestMobileHeader,{passive:true});
    if(menu)menu.addEventListener('click',()=>siteHeader.classList.remove('mobile-header-hidden'));
    updateMobileHeader();
  }
